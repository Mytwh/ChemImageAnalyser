...
def safe_rotate_enlarged_img_size(angle, rows, cols): 
    ...
    # 计算新的宽度和高度
    new_cols = abs(rows * math.sin(angle_radians)) + abs(cols * math.cos(angle_radians))
    new_rows = abs(rows * math.cos(angle_radians)) + abs(cols * math.sin(angle_radians))

    # 返回新的宽度和高度
    return int(new_rows), int(new_cols)
...
class NormalizedGridDistortion(A.augmentations.geometric.GridDistortion):
